Alias: $cpg-knowledgeCapability = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-knowledgeCapability
Alias: $cpg-knowledge-capability-codes = http://hl7.org/fhir/uv/cpg/CodeSystem/cpg-knowledgeCapability
Alias: $cpg-knowledge-capability-valueset = http://hl7.org/fhir/uv/cpg/ValueSet/cpg-knowledge-capability
Alias: $cpg-knowledgeRepresentationLevel = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-knowledgeRepresentationLevel
Alias: $cpg-knowledge-representation-level-codes = http://hl7.org/fhir/uv/cpg/CodeSystem/cpg-knowledge-representation-level
Alias: $cpg-knowledge-representation-level-valueset = http://hl7.org/fhir/uv/cpg/ValueSet/cpg-knowledge-representation-level
Alias: $cpg-publishable-valueset = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-publishablevalueset
Alias: $cpg-publishable-codesystem = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-publishablecodesystem

Alias: $m49.htm = http://unstats.un.org/unsd/methods/m49/m49.htm

Alias: $cpg-activity-type = http://hl7.org/fhir/uv/cpg/CodeSystem/cpg-activity-type
Alias: $cpg-casefeature-pertinence = http://hl7.org/fhir/uv/cpg/CodeSystem/cpg-casefeature-pertinence
Alias: $cpg-casefeature-type = http://hl7.org/fhir/uv/cpg/CodeSystem/cpg-casefeature-type
Alias: $cpg-common-persona = http://hl7.org/fhir/uv/cpg/CodeSystem/cpg-common-persona
Alias: $cpg-common-process = http://hl7.org/fhir/uv/cpg/CodeSystem/cpg-common-process
Alias: $cpg-indicator = http://hl7.org/fhir/uv/cpg/CodeSystem/cpg-indicator
Alias: $cpg-recommendation-direction = http://hl7.org/fhir/uv/cpg/CodeSystem/cpg-recommendation-direction

Alias: $cpg-casefeature-pertinence-valueset = http://hl7.org/fhir/uv/cpg/ValueSet/cpg-casefeature-pertinence
Alias: $cpg-casefeature-type-valueset = http://hl7.org/fhir/uv/cpg/ValueSet/cpg-casefeature-type
Alias: $cpg-recommendation-direction-valueset = http://hl7.org/fhir/uv/cpg/ValueSet/cpg-recommendation-direction

Alias: $cpg-case = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-case
Alias: $cpg-careplan = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-careplan
Alias: $cpg-communicationrequest = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-communicationrequest
Alias: $cpg-immunizationrecommendation = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-immunizationrecommendation
Alias: $cpg-medicationrequest = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-medicationrequest
Alias: $cpg-proposediagnosistask = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-proposediagnosistask
Alias: $cpg-questionnairetask = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-questionnairetask
Alias: $cpg-recorddetectedissuetask = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-recorddetectedissuetask
Alias: $cpg-recordinferencetask = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-recordinferencetask
Alias: $cpg-reportflagtask = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-reportflagtask
Alias: $cpg-servicerequest = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-servicerequest
Alias: $cpg-strategy = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-strategy
Alias: $definition-topic = http://hl7.org/fhir/ValueSet/definition-topic

Alias: $cpg-shareablestructuredef = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-shareablestructuredef
Alias: $cpg-inferenceExpression = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-inferenceExpression
Alias: $cpg-assertionExpression = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-assertionExpression
Alias: $cpg-featureExpression = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-featureExpression
Alias: $cpg-caseFeatureOf = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-caseFeatureOf
Alias: $resource-approvalDate = http://hl7.org/fhir/StructureDefinition/resource-approvalDate
Alias: $resource-effectivePeriod = http://hl7.org/fhir/StructureDefinition/resource-effectivePeriod
Alias: $resource-lastReviewDate = http://hl7.org/fhir/StructureDefinition/resource-lastReviewDate
Alias: $cpg-topic = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-topic
Alias: $cpg-author = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-author
Alias: $cpg-editor = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-editor
Alias: $cpg-reviewer = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-reviewer
Alias: $cpg-endorser = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-endorser
Alias: $cpg-relatedArtifact = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-relatedArtifact

Alias: $cpg-shareableactivitydefinition = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-shareableactivitydefinition
Alias: $shareableactivitydefinition = http://hl7.org/fhir/StructureDefinition/shareableactivitydefinition
Alias: $shareablecodesystem = http://hl7.org/fhir/StructureDefinition/shareablecodesystem
Alias: $cpg-identifier = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-identifier
Alias: $cpg-title = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-title
Alias: $shareablelibrary = http://hl7.org/fhir/StructureDefinition/shareablelibrary
Alias: $shareablemeasure = http://hl7.org/fhir/StructureDefinition/shareablemeasure
Alias: $shareableplandefinition = http://hl7.org/fhir/StructureDefinition/shareableplandefinition
Alias: $cpg-answerValueSetSource = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-answerValueSetSource
Alias: $cpg-itemImage = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-itemImage
Alias: $cpg-responseImage = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-responseImage
Alias: $shareablevalueset = http://hl7.org/fhir/StructureDefinition/shareablevalueset

Alias: $cpg-shareablecodesystem = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-shareablecodesystem
Alias: $resource-approvalDate = http://hl7.org/fhir/StructureDefinition/resource-approvalDate
Alias: $codesystem-effectiveDate = http://hl7.org/fhir/StructureDefinition/codesystem-effectiveDate
Alias: $codesystem-expirationDate = http://hl7.org/fhir/StructureDefinition/codesystem-expirationDate
Alias: $resource-lastReviewDate = http://hl7.org/fhir/StructureDefinition/resource-lastReviewDate
Alias: $codesystem-author = http://hl7.org/fhir/StructureDefinition/codesystem-author
Alias: $codesystem-keyWord = http://hl7.org/fhir/StructureDefinition/codesystem-keyWord
Alias: $codesystem-sourceReference = http://hl7.org/fhir/StructureDefinition/codesystem-sourceReference

Alias: $cpg-shareablegraphdefinition = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-shareablegraphdefinition
Alias: $resource-effectivePeriod = http://hl7.org/fhir/StructureDefinition/resource-effectivePeriod
Alias: $cpg-copyright = http://hl7.org/fhir/uv/cpg/StructureDefinition/cpg-copyright

Alias: $codesystem-concept-comments = http://hl7.org/fhir/StructureDefinition/codesystem-concept-comments









